...
def train_ref_read_anno_residual(args):
	...
	weight_path = arguments.weight_path_from_args(args)
	model = models.read_tensor_2d_annotation_residual_from_args(args, 
									conv_width = 5,
									conv_height = 5,
									conv_layers = [128, 128, 128, 128, 128, 128, 128, 128],
									conv_dropout = 0.0,
									conv_batch_normalize = False,
									kernel_single_channel = True,
									spatial_dropout = True,
									max_pools = [(2,2),(2,2),(2,2),(2,2)],
									padding='same',
									annotation_units = 32,
...
def train_ref_read_residual(args):
	'''Trains a reference and read based architecture on tensors at the supplied data directory.
...
def train_ref_read_anno_tiny(args):
	'''Trains a reference and read based architecture on tensors at the supplied data directory.
...
def score_dict_from_shared_positions(args, kind, shared_positions, cnn_scores, compare_scores):
	...
	for a in args.architectures:
		scores['GATK4_'+td.plain_name(a)] = [cnn_scores[a][p] for p in shared_positions]
